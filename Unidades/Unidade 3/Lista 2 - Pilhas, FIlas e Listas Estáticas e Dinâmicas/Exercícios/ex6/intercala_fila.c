...
int fila_intercalada(fila * queue1, fila * queue2, fila * queue3) {
    if(queue1 == NULL)
    {
        printf("Fila 1 não alocada.\n");
        return FALHA;
    }
    if(queue2 == NULL)
    {
        printf("Fila 2 não alocada.\n");
        return FALHA;
    }
    if(queue3 == NULL)
    {
        printf("Fila 3 não alocada.\n");
        return FALHA;
    }
    int tam_final = tamanho_fila(queue1) + tamanho_fila(queue2);
    if(tam_final > TAM_MAX_FILA)
    {
        printf("A fila intercalada vai ultrapassar o limite de tamanho.\n");
        return FALHA;
    }

    int i = 0, j = 0, k = 0;
    if(tamanho_fila(queue1) >= tamanho_fila(queue2))
    {
        while(1)
        {
            if((i == j || j == tamanho_fila(queue2)) && i < tamanho_fila(queue1))
            {
                queue3->elem[k] = queue1->elem[i];
                i++;
            }
            else if(i > j)
            {
                queue3->elem[k] = queue2->elem[j];
                j++;
            }
            k++;
            queue3->fim++;
            if(i == tamanho_fila(queue1) && j == tamanho_fila(queue2)) break;
        }
    }
    else
    {
        while(1)
        {
            if(i == j && i < tamanho_fila(queue1))
            {
                queue3->elem[k] = queue1->elem[i];
                i++;
            }
            else if(i > j || (i <= j && j < tamanho_fila(queue2)))
            {
                queue3->elem[k] = queue2->elem[j];
                j++;
            }
